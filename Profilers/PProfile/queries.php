          ...
          <div class="row">
            <nav class="templatemo-top-nav col-lg-12 col-md-12">
              <ul class="text-uppercase">
               <li><a href="../../Homepage/index.php">HOME LTCE</a></li>
...
$qu="SELECT DISTINCT count(CompanyName) from addpdrive where PVenue LIKE '%LTCE%' AND YEAR(Date)=YEAR(NOW())";
...
$que="SELECT count(Attendence) from updatedrive where Attendence=1 AND YEAR(Date)=YEAR(NOW())";
$RESULT=mysqli_query($connect,$que);
$data=mysqli_fetch_assoc($RESULT);
echo "<br><BR>Number of Students Attended In This Year&nbsp:&nbsp";
echo $data['count(Attendence)'];
$quer="SELECT count(Placed) from updatedrive where Placed=1 AND YEAR(Date)=YEAR(NOW())";
...
?>		
          ...
          <footer class="text-right">
            <p>Copyright &copy; 2023 LTCE
            | Developed by <a href="http://gpthane.org.in/" target="_parent">LTians</a></p>
          </footer>         
        </div>
      </div>
    </div>    
